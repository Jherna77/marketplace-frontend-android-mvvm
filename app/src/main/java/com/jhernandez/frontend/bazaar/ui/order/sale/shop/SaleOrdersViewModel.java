...
/*
 * ViewModel for managing sale orders for a shop.
 * It handles loading, filtering, and navigation events related to sale orders.
 */
public class SaleOrdersViewModel extends BaseViewModel {

    private final OrderRepositoryPort orderRepository;
    private final MutableLiveData<SaleOrdersViewState> viewState = new MutableLiveData<>();
    ...
    public SaleOrdersViewModel(OrderRepositoryPort orderRepository, SessionRepositoryPort sessionRepository) {
        this.orderRepository = orderRepository;
        this.user = sessionRepository.getSessionUser();
        initViewState();
    }

    public LiveData<SaleOrdersViewState> getViewState() {
        return viewState;
    }
    ...
    public void initViewState() {
        viewState.setValue(new SaleOrdersViewState(false, false));
        selectedStatus = TAG_ALL;
        selectedOrder = TAG_ASCENDING;
        findSaleOrdersByUserId(user.id());
    }
    ...
    public void findSaleOrdersByUserId(Long userId) {
        Log.d("ShopSalesViewModel", "Loading sales for user shop with id " + userId);
        orderRepository.findSaleOrdersByUserId(userId, typeCallback(
                filteredOrders,
                ordersResult -> {
